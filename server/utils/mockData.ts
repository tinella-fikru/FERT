...
const c = (id: string, slug: string, name: string, description: string, season: string, sort: number, hero: string | null = null): Collection => ({
  id, slug, name, description, season, hero_image_url: hero, sort_order: sort, published: true,
})

const IMG = (n: number) => `/images/Designs/photo_${n}_2026-07-18_18-58-29.jpg`

export const MOCK_COLLECTIONS: Collection[] = [
  c('col-1', 'meskel', 'Meskel', 'A celebration collection for the Finding of the True Cross — luminous whites, bonfire golds, garments made for procession and gathering.', 'Meskerem 2019', 1, IMG(1)),
  c('col-2', 'timket', 'Timket', 'Ceremonial dress for Epiphany — flowing netela, layered kemis, and the discipline of white cotton against highland light.', 'Tir 2019', 2, IMG(15)),
  c('col-3', 'atelier-line', 'Atelier Line', 'Everyday habesha dressing — simplified silhouettes, refined menen cotton, tibeb reduced to a single quiet line.', 'Permanent', 3, IMG(20)),
]
...
const g = (
  ...
  base_price_etb: price, lead_time_days: lead, made_to_order: true, image_urls: images,
...
export const MOCK_GARMENTS: Garment[] = [
  ...
  g('gar-4', 1, 'timket-kemis-tabot', 'Tabot Kemis', 'habesha_kemis',
    ...
    21400, 28, 1, [IMG(15), IMG(19), IMG(17)]),
...
const t = (id: string, slug: string, name: string, description: string, delta: number): TibebPattern => ({
  id, slug, name, description, story: null, price_delta_etb: delta,
  image_url: null, image_urls: [], available: true,
})
